...
def logout():
	logout_user()
	flash('You have been logged out', 'flashMessage')
...
def register():
	...
	if form.validate_on_submit():
		...
		flash('A confirmation email has been sent to you by email', 'flashMessage')
		return redirect(url_for('main.index'))
	elif form.errors.items():
		for field, errors in form.errors.items():
			flash(field + ": " + errors[0], 'flashMessage_error')
...
def confirm(token):
	...
	if current_user.confirm(token):
		flash('You have confirmed your accounts. Thanks!', 'flashMessage')
	else:
...
def resend_confirmation():
	...
	flash('A new confirmation email has been sent to you by email.', 'flashMessage')
	return redirect(url_for('main.index'))
...
def password_reset_request():
	...
	if form.validate_on_submit():
		...
		if user:
			...
			flash('An email with instructions to reset your password has been sent to you.', 'flashMessage')
...
def password_reset(token):
	...
	if form.validate_on_submit():
		if user.reset_password(token, form.password.data):
			flash('Your password has been updated.', 'flashMessage')
			return redirect(url_for('auth.login'))
		else:
			flash('The reset link is invalid or has expired.', 'flashMessage_error')
			return redirect(url_for('main.index'))
